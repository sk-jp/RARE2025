...
class ImageLabelDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        ...
        self.dataset = {}
        self.dataset['train'] = ImageLabelDataset(
            cfg.Data.dataset.top_dir,
            cfg.Data.dataset.train_datalist,
            transform=get_transform(cfg.Transform['train']))
        self.dataset['valid'] = ImageLabelDataset(
            cfg.Data.dataset.top_dir,
            cfg.Data.dataset.valid_datalist,
            transform=get_transform(cfg.Transform['valid']))
        if self.cfg.General.mode == "predict":
            self.dataset['predict'] = ImageLabelDataset(
                cfg.Data.dataset.top_dir,
                cfg.Data.dataset.predict_datalist,
                transform=get_transform(cfg.Transform['predict']))
